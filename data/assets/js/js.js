...
function populate_bank(category) {
    emojione.imageType = 'png';
    var bank = [smiles, nature, objects, places, other];
    var category_emojis = bank[category];
    category_emojis = category_emojis.split(" ");
    var constructed_html = "";
    var i = 0;
    for (emoji in category_emojis) { //iterate through bank icons
        var bank_icon = "<li class='emoji'>" + emojione.shortnameToImage(category_emojis[emoji]) + "</li>";
        constructed_html += bank_icon;
    }
    $('#bank').html(constructed_html);
}
...
$(document).ready(function() {
    ...
    $('.warm').append(emojione.shortnameToImage(":hash:"));

    var slot_cursor = -1; //cursor

    $('#bank').on('click', '.emoji', function(e) {
        if (generated == true)
            return;
        slot_cursor = find_next_slot();

        $('.slot').removeClass('locked');
        $('.slot:eq(' + slot_cursor + ')').empty();


        emojione.imageType = 'svg';
        $('.slot:eq(' + slot_cursor + ')').append(
        ...
        if (slot_cursor < 5) {
            $('.slot').removeClass('active');
            $('.slot:eq(' + (slot_cursor + 1) + ')').addClass('active');
        } else {
            e.preventDefault();
            e.stopPropagation();
            generate();
    ...
    $('.slot').click(function() {
        $('.slot').removeClass('active locked');
